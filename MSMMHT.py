...
class MyEnsemble(nn.Module):
    def __init__(self, model1, model2, model3, model4, model5, model6, model1t, model2t, model3t, model1_2, model2_1, model1_3, model3_1, model2_3, model3_2, model1_1, model2_2, model3_3, att_layer, att_layer2):
        ...
        self.linear = nn.Linear(2048,4).float().to(device)
    ...
    def forward(self, src1, src2, src3, src_att1, src_att2, src_att3, target_input, trg_att):
        ...
        predt2 = self.lineart2(target_input)
        predt3 = self.lineart3(target_input)
...
for include_prior in include_prior_list:
    ...
    for history_sec in history:
        ...
        for future_pred in future:
                ...
                model4=TransformerEncoder(512, heads, layers, attn_mask=True).float().to(device)
                model5=TransformerEncoder(512, heads, layers, attn_mask=True).float().to(device)
                model6=TransformerEncoder(512, heads, layers, attn_mask=True).float().to(device)
                ...
                model1_2=TransformerEncoder(1024, heads2, layers).float().to(device)
                model2_1=TransformerEncoder(1024, heads2, layers).float().to(device)
                model1_3=TransformerEncoder(1024, heads2, layers).float().to(device)
                model3_1=TransformerEncoder(1024, heads2, layers).float().to(device)
                model2_3=TransformerEncoder(1024, heads2, layers).float().to(device)
                model3_2=TransformerEncoder(1024, heads2, layers).float().to(device)
                model1_1=TransformerEncoder(1024, heads2, layers).float().to(device)
                model2_2=TransformerEncoder(1024, heads2, layers).float().to(device)
                model3_3=TransformerEncoder(1024, heads2, layers).float().to(device)
