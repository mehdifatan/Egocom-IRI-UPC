...
class TVAModel_Self(nn.Module): # trimodal self-attn model  
    def __init__(self, params):
        ...
        params.txt_rnnsize=25
        params.rnnsize=50
        params.vid_rnnsize=50
        params.txt_rnnsize=50
        params.aud_rnnsize=50
        
        #print(params.rnndir)
        #print(params.txt_rnnnum)
        

        rnn = nn.LSTM if params.rnntype == "lstm" else nn.GRU
        self.text_encoder = rnn(input_size=params.txt_dim, hidden_size=50,
        ...
        self.video_conv = nn.Conv1d(in_channels=10, out_channels=10, kernel_size=1) # 825 params
        ...
        self.video_encoder = rnn(input_size=params.vid_dim, hidden_size=params.vid_rnnsize,
                                 ...
                                 batch_first=True)
        self.audio_conv = nn.Conv1d(in_channels=10, out_channels=10, kernel_size=1)
        self.audio_encoder = rnn(input_size=params.aud_dim, hidden_size=params.aud_rnnsize,
                                 ...
                                 batch_first=True)
        if params.rnndir:
            self.mha_t = nn.MultiheadAttention(embed_dim=params.rnnsize*2, num_heads=params.txt_nh,
                                                 dropout=params.txt_dp, batch_first=True)
            self.mha_v = nn.MultiheadAttention(embed_dim=params.rnnsize*2, num_heads=params.vid_nh,
                                               dropout=params.vid_dp, batch_first=True)
            self.mha_a = nn.MultiheadAttention(embed_dim=params.rnnsize*2, num_heads=params.aud_nh,
                                               dropout=params.aud_dp, batch_first=True)
            self.concat_linear = nn.Linear(in_features=200, out_features= 20)
            self.classifier = nn.Linear(in_features= 20, out_features=params.output_dim)

    def forward(self, x_txt, x_vid, x_mfcc, emb_dp=0.25):
        #x_txt = torch.unsqueeze(x_txt, dim=0)
        #x_vid = torch.unsqueeze(x_vid, dim=0)
        #x_mfcc = torch.unsqueeze(x_mfcc, dim=0)
        # text branch
        ...
        x_tva1 = torch.stack((x_txt2, x_vid2, x_mfcc2), dim=2)
        
        #print(x_tva1.size())
        
        x_tva1_mean, x_tva1_std = torch.std_mean(x_tva1, dim=2)

        
        #print(x_tva1_mean.size())
        #print(x_tva1_std.size())

        x_tva = torch.cat((x_tva1_mean, x_tva1_std), dim=2) 
        
        #print("x_tva",x_tva.size())
...
class TVAModel_Cross(nn.Module): # trimodal cross-attn model
    def __init__(self, params):
        ...
        self.text_encoder = rnn(input_size=params.txt_dim, hidden_size=params.txt_rnnsize,
                                num_layers=params.txt_rnnnum, dropout=params.txt_rnndp, bidirectional=params.rnndir,
                                batch_first=True)
        self.video_conv = nn.Conv1d(in_channels=25, out_channels=25, kernel_size=1) # 825 params
        self.video_encoder = rnn(input_size=params.vid_dim, hidden_size=params.vid_rnnsize,
                                 num_layers=params.vid_rnnnum, dropout=params.vid_rnndp, bidirectional=params.rnndir,
                                 batch_first=True)
        self.audio_conv = nn.Conv1d(in_channels=25, out_channels=25, kernel_size=1)
        self.audio_encoder = rnn(input_size=params.aud_dim, hidden_size=params.aud_rnnsize,
                                 num_layers=params.aud_rnnnum, dropout=params.aud_rnndp, bidirectional=params.rnndir,
                                 batch_first=True)
        if params.rnndir:
